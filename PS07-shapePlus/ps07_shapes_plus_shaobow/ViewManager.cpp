...
void ViewManager::load()
{
	...
	if (inFile.is_open()) {
		...
		if (inFileName.find("Lshape") != string::npos)
			theShape = new Lshape();
		else if(inFileName.find("Ishape") != string::npos)
			theShape = new Ishape();
		else
			theShape = new Shape();
...
bool ViewManager::manage()
{
	...
	// always display properties image exists
...
void ViewManager::showMenu()
{
	...
	cout << endl;
	cout << "    P : calculate some section properties of a shape" << endl;
