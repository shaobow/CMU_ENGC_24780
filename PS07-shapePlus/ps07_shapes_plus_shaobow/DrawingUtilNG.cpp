...
double DrawingUtilNG::getIntersection(double& resultX, double& resultY, 
	double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4)
{
	// note how I switched the parameter names above 
	// so that I can use the wikipedia equations directly
	// https://en.wikipedia.org/wiki/Line%E2%80%93line_intersection
	double denominator = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4);
	if (abs(denominator) < 1e-8) { // denominator == 0.
		resultX = -INFINITY;
		resultY = -INFINITY;
	}
	else {
		resultX = ((x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2)*(x3 * y4 - y3 * x4)) / denominator;
		resultY = ((x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2)*(x3 * y4 - y3 * x4)) / denominator;
	}
	return 0.0;
}
