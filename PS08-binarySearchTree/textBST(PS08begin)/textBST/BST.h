...
inline int BST<T, TN>::retrieveItemCount(const T& anItem)
{
	int location = findItem(anItem);
	if (location > 0)
		return theTree.at(location).count;
	else
		return 0;
}
...
{
	if (startLoc < theTree.size() && theTree.at(startLoc).count>0) {
		if (anItem == theTree.at(startLoc).theItem)
			return startLoc;
		else if (anItem < theTree.at(startLoc).theItem)
			return findItem(anItem, 2 * startLoc + 1);
		else
			return findItem(anItem, 2 * startLoc + 2);
	}
